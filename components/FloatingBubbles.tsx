...
//   const getElapsedSeconds = (start: string) => Math.floor((tick - new Date(start).getTime()) / 1000);
//   const formatTime = (seconds: number) => {
//     const m = Math.floor(seconds / 60);
//     const s = seconds % 60;
//     return `${m}:${s.toString().padStart(2, "0")}`;
...
//                   onClick={canClaimBonus ? triggerConfetti : undefined}
//                   className={`relative flex flex-col items-center justify-center w-24 h-24 rounded-full shadow-2xl cursor-pointer border-4 border-white dark:border-gray-800 text-white
//                     ${canClaimBonus ? "bg-gradient-to-br from-yellow-400 to-orange-500 animate-bounce" : "bg-gradient-to-br from-pink-500 to-purple-600"}
//                   `}
//                 >
//                   <Gift className="w-8 h-8 mb-1" />
//                   <span className="text-xs font-black uppercase tracking-tighter">
...
//                 </div>
//               </PopoverTrigger>
//               <PopoverContent side="left" className="w-64 p-4 rounded-2xl text-center">
//                 {canClaimBonus ? (
//                   <div className="space-y-2">
//                     <h3 className="font-bold text-lg text-orange-600">Félicitations ! 🥳</h3>
//                     <p className="text-sm">Vous avez parrainé 5 amis. Allez dans l'onglet <b>Parrainage</b> pour réclamer votre bonus !</p>
...
//           return (
//             <motion.div key={appt.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}>
//               <Popover>
//                 <PopoverTrigger asChild>
//                   <div className={`text-white px-4 py-3 rounded-2xl shadow-xl w-32 flex flex-col items-center justify-center cursor-pointer
//                     ${user.role === "worker" && exceeded ? "bg-red-500" : "bg-purple-600"} ${isActive ? "animate-pulse" : ""}`}>
//                     <p className="text-lg font-bold">{formatTime(elapsedSec)}</p>
//                     <span className="text-[10px] opacity-80 uppercase font-bold">En cours</span>
//                   </div>
//                 </PopoverTrigger>
//                 <PopoverContent className="w-64 p-4 rounded-2xl">
//                   <p className="font-semibold text-sm">{appt.service?.name}</p>
//                   <p className="text-xl text-gray-500">{appt.client?.user?.name}</p>
//                 </PopoverContent>
//               </Popover>
//             </motion.div>
//           );
