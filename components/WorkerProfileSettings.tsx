  ...
  useEffect(() => {
    if (workerProfile) {
      setFormData({
        ...
        isActive: workerProfile.user?.isActive!,
        ...
        <Card className="p-6 border border-pink-100 dark:border-pink-900/30 bg-white dark:bg-gray-950">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              <Avatar className="h-20 w-20">
                <AvatarImage
                  src={workerProfile?.user?.avatar || ""}
                  alt={workerProfile?.user?.name}
                />
                <AvatarFallback>
                  {workerProfile?.user?.name?.charAt(0)}
                </AvatarFallback>
              </Avatar>
          ...
          <div className="mt-4 flex flex-row justify-between">
            ...
            <div className="">
              {user?.role === "admin" && (
                <>
                  <Label>Desactivé ce compte</Label>
                  <div className="flex items-center gap-2">
                    <Switch
                    ...
                    />
                    <span className="text-sm text-gray-600 dark:text-gray-400">
                      {formData.isActive ? "Activé" : "Desactivé"}
                    </span>
                  </div>
                </>
              )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    handleInputChange("commissionFrequency", value as any)
                  }
                  disabled={!showCommissionSettings && isCommissionLocked}
                >
                  <SelectTrigger>
