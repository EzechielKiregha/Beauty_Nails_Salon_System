...
export async function POST(
  ...
  try {
    ...
    if (appointment.workerId !== user.workerProfile?.id) {
      return errorResponse('Vous ne pouvez transférer que vos propres rendez-vous', 403);
    }
    ...
    if (conflict) {
      return errorResponse('Cet employé a déjà un rendez-vous à ce créneau', 409);
    }
    ...
    return successResponse({ transfer, message: 'Demande de transfert envoyée' });
  } catch (error) {
    return handleApiError(error);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    if (transfer.newWorkerId !== user.workerProfile?.id) {
      return errorResponse('Vous ne pouvez répondre qu\'aux transferts qui vous sont destinés', 403);
    }
    ...
    return successResponse({ transfer: updatedTransfer, message: `Transfert ${action === 'accept' ? 'accepté' : 'refusé'}` });
  } catch (error) {
    return handleApiError(error);
  }
}